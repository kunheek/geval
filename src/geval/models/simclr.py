# Adapted from https://github.com/Separius/SimCLRv2-Pytorch
# Requires first downloading and converting the tensorflow checkpoint by following the instructions at the github
# we used model r50_1x_sk1
# Then call geval with --model sinclr --checkpoint /path/to/simclr_checkpoint
import torch
...
class SimCLRResNetEncoder(Encoder):
    def setup(self, ckpt):
        ckpt = '/home/george/src/SimCLRv2-Pytorch/r50_1x_sk1.pth'
        self.model, _ = get_resnet(*name_to_params(ckpt))
        self.model.load_state_dict(torch.load(ckpt)['resnet'])
