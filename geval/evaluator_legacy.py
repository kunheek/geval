...
tf.config.experimental.enable_tensor_float_32_execution(False)  # NOTE: disable TF32 on Ampere GPUs
...
def main():
    ...
    ref_acts_pkl = args.ref_batch.replace(".npz", "_acts.pkl")
    if os.path.isfile(ref_acts_pkl):
        print("reading reference batch activations...")
        with open(ref_acts_pkl, "rb") as f:
            ref_acts = pickle.load(f)
    else:
        print("computing reference batch activations...")
        ref_acts = evaluator.read_activations(args.ref_batch)
        with open(ref_acts_pkl, "wb") as f:
            pickle.dump(ref_acts, f)
    ...
    inception_score, is_std = evaluator.compute_inception_score(sample_acts[0])
    print(f"Inception Score: {inception_score:.3f} {chr(177)} {is_std:.3f}")
    ...
    with open(args.csv_file, 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        cmd_parse = args.sample_batch.split('/')
        refname = os.path.basename(args.ref_batch)
        datasetname, expname, npzname = cmd_parse[-4], cmd_parse[-3], cmd_parse[-1]

        is_str = f"{inception_score:.3f}+/-{is_std:.3f}"
        writer.writerow([
            refname, datasetname, expname, npzname,
            is_str, f"{fid:.2f}", f"--.--", f"{prec:.2f}", f"{recall:.2f}",
            "tensorflow",
        ])
